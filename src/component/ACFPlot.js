...
const ACFPlot = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post('http://siddhishah.pythonanywhere.com/acf', {
          data: data,
          lags: lag 
        });
        setACFValues(response.data.acf_values);
      } catch (error) {
  ...
  const chartData = acfValues.map((value, index) => ({ [xDataKey]: index, [dataKey]: value.toFixed(3) }));
